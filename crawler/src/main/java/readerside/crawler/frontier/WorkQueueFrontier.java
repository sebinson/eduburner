...
public class WorkQueueFrontier extends AbstractFrontier implements ApplicationContextAware{
	...
	protected CrawlURI findEligibleURI() {
		...
		do{
			...
			if(readyQ.getCount() == 0){
                logger.debug("ready queue contains no active uri.");
				readyQ.clearHeld();
				readyQ = null;
			}
		}while(readyQ == null);
	...
	protected void processFinish(CrawlURI uri) {
        logger.debug("entering processFinish method. uri: " + uri.getUrl());
		...
		if (wq != null) {
			...
			long delay = getMinCrawlInterval();
			//wq.enqueue(this, uri);
	...
	protected void sendToQueue(CrawlURI curi) {
        ...
        logger.debug("entering sendToQueue method...");
